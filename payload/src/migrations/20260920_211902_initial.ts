...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`pages\` (
  	...
  	FOREIGN KEY (\`footer_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`pages_slug_idx\` ON \`pages\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`pages_footer_image_idx\` ON \`pages\` (\`footer_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_pages_v\` (
  	...
  	FOREIGN KEY (\`version_footer_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_parent_idx\` ON \`_pages_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_version_version_slug_idx\` ON \`_pages_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_version_version_footer_image_idx\` ON \`_pages_v\` (\`version_footer_image_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`events_start_date_idx\` ON \`events\` (\`start_date\`);`)
  await db.run(sql`CREATE INDEX \`events_image_idx\` ON \`events\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`events_archive_after_idx\` ON \`events\` (\`archive_after\`);`)
  ...
  await db.run(sql`CREATE INDEX \`members_order_idx\` ON \`members\` (\`order\`);`)
  ...
  await db.run(sql`CREATE TABLE \`members_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  ...
  `)
  await db.run(sql`CREATE INDEX \`members_rels_order_idx\` ON \`members_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`members_rels_parent_idx\` ON \`members_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`members_rels_path_idx\` ON \`members_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`members_rels_media_id_idx\` ON \`members_rels\` (\`media_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`faq_order_idx\` ON \`faq\` (\`order\`);`)
  ...
  await db.run(sql`CREATE TABLE \`media\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`alt\` text NOT NULL,
  	\`focus_x\` numeric DEFAULT 50,
  	\`focus_y\` numeric DEFAULT 50,
  	\`credit\` text,
  	\`_objectkey\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE INDEX \`header_items_link_link_page_idx\` ON \`header_items\` (\`link_page_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`header_cta_link_cta_link_page_idx\` ON \`header\` (\`cta_link_page_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`footer_items_link_link_page_idx\` ON \`footer_items\` (\`link_page_id\`);`)
  await db.run(sql`CREATE TABLE \`footer_legal_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`link_label\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_legal_items_order_idx\` ON \`footer_legal_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`footer_legal_items_parent_id_idx\` ON \`footer_legal_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`footer_legal_items_link_link_page_idx\` ON \`footer_legal_items\` (\`link_page_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_background_image_idx\` ON \`footer\` (\`background_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`site_name\` text DEFAULT 'Mellowbikers' NOT NULL,
  	\`default_og_image_id\` integer,
  	\`logo_id\` integer,
  	\`email\` text,
  	\`area_served\` text DEFAULT 'Bergen op Zoom, Brabantse Wal',
  	\`updated_at\` text,
  	\`created_at\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`members_rels\`;`)
  ...
  await db.run(sql`DROP TABLE \`footer_legal_items\`;`)
